...
pub async fn upscale_single_image(
    ...
    let models_folder = generate_command_parameters!();
    ...
    let command = tauri::async_runtime::spawn(async move {
        ...
        let (mut rx, mut _child) = match Command::new_sidecar("esrgan")
            .expect("Failed to load ESRGAN binary")
            .args(command_args).spawn() {
